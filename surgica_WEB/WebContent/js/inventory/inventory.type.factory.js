angular.module('hb.smartcard.factory.InventoryType', [])

.factory('InventoryTypeFactory', function ( $firebase , $q , $log, $timeout )
{
	var factory = {};
	var ref = new Firebase("https://surgica.firebaseio.com/inventory/types/");
	var inventoryTypeComparator = function(a,b) {
	...
	factory.findAll = function findAll() 
	{
		...
		items.sort(inventoryTypeComparator);

		return items;
	};
	
	factory.initialize = function initialize() {
		inventoryTypes.sort(inventoryTypeComparator);
		var sorted = inventoryTypes.slice(0);
